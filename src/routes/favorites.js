...
router.get('/', async (req, res) => {
  try {
    const favorites = await fetchMany('favorites', {
      filters: [{ column: 'userId', operator: 'eq', value: req.user.id }],
      orderBy: 'createdAt',
      ascending: false,
    });
    res.json(favorites);
  ...
  try {
    ...
    const existing = await fetchMany('favorites', {
      filters: [
        ...
        { column: 'inventoryId', operator: 'eq', value: itemId },
      ],
      limit: 1,
    });
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    if (!favorite || String(favorite.userId) !== String(req.user.id)) {
  ...
  try {
    const favorites = await fetchMany('favorites', {
      filters: [
        ...
        { column: 'inventoryId', operator: 'eq', value: req.params.itemId },
      ],
      limit: 1,
    });
    const favorite = favorites[0];
    res.json({ isFavorite: !!favorite, favoriteId: favorite?.id || favorite?._id });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
});
